...
const Channel = () => {
   ...
   const { data: chatData, mutate: mutateChat, setSize } = useSWRInfinite<IChat[]>(
    (index: number) => `/api/workspaces/${workspace}/channels/${channel}/chats?perPage=20&page=${index + 1}`, fetcher,
   )
   ...
   const onSubmitForm = useCallback((e: { preventDefault: () => void; }) => {
      ...
      if (chat?.trim() && chatData && channelData) {
         ...
         mutateChat((prevChatDate) => {
            prevChatDate?.[0].unshift({
  ...
  useEffect(() => {
   ...
   if (e.dataTransfer.items) {
    ...
    const onDragOver = useCallback((e: { preventDefault: () => void; }) => {
     e.preventDefault()
     setDragOver(true);
    }, [])
